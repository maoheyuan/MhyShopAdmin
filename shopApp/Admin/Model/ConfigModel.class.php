...
class ConfigModel extends Model {
    ...
    public function getInfoByConfigName($memberName){
        $map=array();
        $map["config_name"]=$memberName;
        $memberInfo=$this->where($map)->find();
        return $memberInfo;
    }

    public  function  getList($map,$orderBy="config_id desc",$limit=25){
    ...
    public  function exportList($map=array(),$orderBy="config_ desc"){
   ...
   public function configAddOrEditRules(){
       $rules = array(
           array('config_id'      ,  'require'   ,   '编号不能为空!', self::MODEL_UPDATE),
    ...
    public  function  configAddOrEditData($type){
        $data = array(
    ...
    public function returnData($status,$tip,$data){
     ...
     public  function configAdd(){
        $rules=$this->configAddOrEditRules();
        $data=$this->configAddOrEditData("add");
    ...
    public  function  configEdit(){
        $rules=$this->configAddOrEditRules();
        $data=$this->configAddOrEditData("edit");
        if (!$this->validate($rules)->create($data)){
            return $this->returnData(0,$this->getError(),"");
        }
        $map=array();
        $map["config_id"]=I("post.config_id");
        $result=$this->where($map)->save($data);
        if(!$result){
            return $this->returnData(0,"修改失败!","");
        }
        return $this->returnData(1,"修改成功!",I("post.config_id"));
    }
    public  function  configDelete(){
        $rules = array(
            array("config_id", "require","会员编号不能为空!")
        );
        $data = array(
        ...
        $map = array();
        $map["config_id"] = I("get.config_id");
        $result = $this->where($map)->delete();
        if (!$result) {
            return $this->returnData(0,"册除失败!","");
        }
        return $this->returnData(1,"册除成功!",I("get.config_id"));
    }
}