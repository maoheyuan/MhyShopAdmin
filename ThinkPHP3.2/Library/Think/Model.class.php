...
class Model {
    ...
    // 链操作方法列表
    protected $methods          =   array('strict','order','alias','having','group','lock','distinct','auto','filter','validate','result','token','index','force');//,'dataBaseType'
    /**
     * 架构函数
     * 取得DB类的实例对象 字段检查
     * @access public
...
/*        var_dump($connection);
     ...
     * @auth byron sampson
     */
   /* public function dataBaseType($type="slave",$number=0){
        $deploy    = C('DB_DEPLOY_TYPE');
        // 如果手动指定主库读取，并且开启了分布式数据库支持
        if($type=="slave"&&!empty($deploy)){
            $this->db(0,"", true);
        }
        // 如果手动指定主库读取，并且开启了分布式数据库支持
        if ($type=="master" && !empty($deploy)){
            // 分布式数据库支持
            ...
            //$number     =   floor(mt_rand(0,C('DB_MASTER_NUM')-1));
            $db_master  =   array(
                'DB_TYPE'    =>  C('DB_TYPE'),
                'DB_USER'    =>  isset($_config['username'][$number])?$_config['username'][$number]:$_config['username'][0],
                'DB_PWD'    =>  isset($_config['password'][$number])?$_config['password'][$number]:$_config['password'][0],
                'DB_HOST'    =>  isset($_config['hostname'][$number])?$_config['hostname'][$number]:$_config['hostname'][0],
                'DB_PORT'    =>  isset($_config['hostport'][$number])?$_config['hostport'][$number]:$_config['hostport'][0],
                'DB_NAME'    =>  isset($_config['database'][$number])?$_config['database'][$number]:$_config['database'][0],
                'DB_CHARSET'   =>  isset($_config['charset'][$number])?$_config['charset'][$number]:$_config['charset'][0],
            );
  ...
  /*  public   function getMasterNumber(){
        return floor(mt_rand(0,C('DB_MASTER_NUM')-1));
    }*/
